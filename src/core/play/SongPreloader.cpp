...
namespace rhythmreplugged::core
{
	...
	void SongPreloader::begin(const std::string &song_directory)
	{
		...
		std::vector<StemJob> stem_jobs;
		stem_jobs.reserve(kPlayableStemNames.size());
		for (size_t order_index = 0; order_index < kPlayableStemNames.size(); ++order_index)
		{
			...
			stem_jobs.push_back({order_index, stem_name, stem_path});
		}

		auto request_state = std::make_shared<RequestState>();
		request_state->song_directory = song_directory;
		request_state->metadata = make_song_metadata_view(parse_result.metadata, folder_name_from_path(song_directory));
		request_state->total_stem_count = stem_jobs.size();
		request_state->stems.reserve(stem_jobs.size());
		request_state->stem_order_indices.reserve(stem_jobs.size());
		...
		if (!worker_.running())
			worker_.start(BackgroundWorker::automatic_thread_count(stem_jobs.size()));

		for (const StemJob &job : stem_jobs)
		{
			if (!worker_.enqueue([this, generation, request_state, job]()
			{
				if (generation_.load() != generation || request_state->failed.load())
					return;

				const auto stem_bytes = file_system_.read_binary_file(job.stem_path);
				if (!stem_bytes.has_value())
				{
					request_state->failed.store(true);
					std::scoped_lock lock(request_state->result_mutex);
					request_state->error_message = "Could not read " + job.stem_name + ".ogg.";
					return;
				}

				request_state->total_bytes.fetch_add(stem_bytes->size());
				size_t last_reported_bytes = 0;
				PrototypePlayer::PreloadedStemTrack track;
				track.stem_name = job.stem_name;
				std::string error_message;
				if (!PrototypePlayer::decode_preloaded_stem(*stem_bytes, track, error_message,
					[&](size_t processed_bytes, size_t)
					{
						const size_t clamped_bytes = (std::min)(processed_bytes, stem_bytes->size());
						if (clamped_bytes <= last_reported_bytes)
							return;

						request_state->processed_bytes.fetch_add(clamped_bytes - last_reported_bytes);
						last_reported_bytes = clamped_bytes;
					}))
				{
					request_state->failed.store(true);
					std::scoped_lock lock(request_state->result_mutex);
					request_state->error_message = "Failed to decode " + job.stem_name + ".ogg: " + error_message;
					return;
				}

				{
					std::scoped_lock lock(request_state->result_mutex);
					request_state->stem_order_indices.push_back(job.order_index);
					request_state->stems.push_back(std::move(track));
				}

				const size_t completed_count = request_state->completed_stem_count.fetch_add(1) + 1;
				if (completed_count == request_state->total_stem_count)
					finalize_request(generation, request_state);
			}))
			{
				request_state->failed.store(true);
				std::scoped_lock lock(request_state->result_mutex);
				request_state->error_message = "Could not queue stem preload work.";
				return;
			}
		}
	}
	...
	SongPreloadStatus SongPreloader::status() const
	{
		...
		result.processed_bytes = request_state->processed_bytes.load();
		result.total_bytes = request_state->total_bytes.load();
		result.completed_stem_count = request_state->completed_stem_count.load();
