...
namespace rhythmreplugged::core
{
	...
	public:
		struct GameplayLaneRuntimeState
		{
			...
			int lock_start_measure_index = -1;
			int lock_end_measure_index = -1;
			double lock_start_time_seconds = 0.0;
			double lock_end_time_seconds = 0.0;
			int ready_lock_start_measure_index = -1;
			int ready_lock_end_measure_index = -1;
			...
			int count_eligible_measure_index = 0;
	...
	private:
		...
		struct GameplayLaneDefinition
		{
			...
			std::vector<double> measure_boundaries_seconds;
		...
		void set_replugged_lock_schedule_by_measure(size_t lane_index, int lock_start_measure_index, int lock_end_measure_index);
		void sync_replugged_lane_schedule_times(size_t lane_index);
		...
		void cache_measure_boundaries(GameplayLaneDefinition &lane);
		void set_transient_gameplay_status_message(std::string message, double duration_seconds);
		int replugged_measure_index_for_time(size_t lane_index, double song_time_seconds) const;
		double replugged_measure_start_time(size_t lane_index, int measure_index) const;
		std::optional<std::pair<double, double>> replugged_section_range_for_measure_index(size_t lane_index, int measure_index) const;
		int next_replugged_measure_index(int measure_index) const;
		int previous_replugged_measure_index(int measure_index) const;
		...
		std::optional<std::pair<double, double>> next_replugged_section_range(size_t lane_index, double measure_end_seconds) const;
		std::optional<std::pair<double, double>> previous_replugged_section_range(size_t lane_index, double measure_start_seconds) const;
		...
		bool any_other_lane_is_available_in_measure(size_t excluded_lane_index, int measure_index, double measure_start_seconds, double measure_end_seconds) const;
		...
		std::string transient_gameplay_status_message_;
		double transient_gameplay_status_until_seconds_ = -1.0;
		bool replugged_keep_busy_engaged_ = false;
