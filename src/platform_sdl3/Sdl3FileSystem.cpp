...
namespace rhythmreplugged
{
	...
	std::string Sdl3FileSystem::canonicalize_path(const std::string &path) const
	{
	...
	std::string Sdl3FileSystem::parent_path(const std::string &path) const
	{
		return std::filesystem::path(path).parent_path().generic_string();
	}

	bool Sdl3FileSystem::path_exists(const std::string &path) const
	{
		std::error_code error_code;
		return std::filesystem::exists(std::filesystem::path(path), error_code) && !error_code;
	}

	bool Sdl3FileSystem::path_is_directory(const std::string &path) const
	{
		std::error_code error_code;
		return std::filesystem::is_directory(std::filesystem::path(path), error_code) && !error_code;
	}

	std::vector<RetroDirectoryEntry> Sdl3FileSystem::list_directory(const std::string &path) const
	{
	...
	std::optional<std::string> Sdl3FileSystem::read_text_file(const std::string &path) const
	{
	...
	std::optional<std::vector<std::uint8_t>> Sdl3FileSystem::read_binary_file(const std::string &path) const
	{
		std::ifstream stream(std::filesystem::path(path), std::ios::binary);
		if (!stream)
			return std::nullopt;
